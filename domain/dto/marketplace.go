...
func (u *MarketplaceUpdates) Validate() error {
	if u == nil {
		return errors.New(errors.BadRequest, "marketplace updates cannot be nil")
	}
	if len(u.ToFields()) == 0 {
		return errors.New(errors.BadRequest, "marketplace updates must have at least one field")
	}
	if u.Name != nil {
		if len(*u.Name) <= 0 || len(*u.Name) > 50 {
			return errors.New(errors.BadRequest, "name must be between 1 and 50 characters long")
		}
	}
	return nil
}
